  ...
  def all
    ...
    render :json => users.select { |u|
      ...
      queries.each do |query|
        if !((!u.firstname.nil? && u.firstname.downcase.start_with?(query)) || (!u.middlename.nil? && u.middlename.downcase.start_with?(query)) || (!u.lastname.nil? && u.lastname.downcase.start_with?(query)) ||(!u.email.nil? && u.email.downcase.start_with(query))) then
          rval = false
        end
      end
      rval
